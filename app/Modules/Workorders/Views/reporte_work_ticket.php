...
if ($workorderEquipment) foreach ($workorderEquipment as $r) $totalEquipment += $r['value'];
if ($workorderPersonal)  foreach ($workorderPersonal  as $r) $totalPersonal  += $r['value'];
if ($workorderOcasional) foreach ($workorderOcasional as $r) $totalOcasional += $r['value'];
if ($workorderMaterials) foreach ($workorderMaterials as $r) $totalMaterials += $r['value'];
if ($workorderReceipt)   foreach ($workorderReceipt   as $r) $totalReceipts  += $r['value'];
...
$eqArr  = $workorderEquipment ?: [];
$mpArr  = $workorderPersonal  ?: [];
$subArr = $workorderOcasional ?: [];
$matArr = $workorderMaterials ?: [];
$recArr = $workorderReceipt   ?: [];
...
// ── Cell style helpers ───────────────────────────────────────────────────────
$base  = "border:1px solid $border; padding:3px 5px; font-size:7pt;";
...
$html .= '<td style="width:3%; border:1px solid ' . $border . '; text-align:center; font-weight:bold;'
       ...
       . 'W<br>o<br>r<br>k<br>&nbsp;<br>T<br>i<br>c<br>k<br>e<br>t</td>';

$html .= '<td style="width:97%; border:0; padding:0; vertical-align:top;">';
...
$html .= '</tr>';
$html .= '<tr>';
$html .= '<td style="' . $boldC . ' font-size:11pt;">' . esc($info[0]['id_workorder']) . '</td>';
